...
if (group==FALSE) {
  ...
  #input.data = input.data[order(input.data$Gene_set), ]
...
######################################################################################
### Convert values to scientific notations
######################################################################################

convert2scientific = function(x) { 

    out = gsub("1e", "10^{", format(x), fixed=TRUE)
...
convert_x_log10 = function(name=NULL, 
                           power=seq(-10, 20), 
                           ...
                           ) {
    ...
    scale_x_log10(name, 
                  breaks=breaks10, 
                  labels=convert2scientific(breaks10), 
                  ...
                  )
}

convert_y_log10 = function(name=NULL, 
                           power=seq(-10, 20), 
                           ...
                           ) { 
...
compute_barplot = function(x,
    ...
    png(paste0(output, paste0(event, "_", set, "_terms_barplot.png")),
        width=ifelse(group==TRUE, 20, 11),
        height=ifelse(group==TRUE, 18, 7),
        units="in", res=600)
    print(plot)
    dev.off()
