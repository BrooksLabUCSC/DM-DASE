...
parser$add_argument("-i", "--input", type="character", required=TRUE, help="Path to containing Delta PSI values")
...
parser$add_argument("-f", "--psi_threshold", required=TRUE, help="Specify p-adj threshold for gene labels") 
...
fold_threshold = args$psi_threshold[1]  
...
custom_volcano = function(x,  
    ...
    x = x[!is.infinite(x$deltaPSI), ]
     
    volcano = ggplot(x, aes(x = deltaPSI, y = -log10(gene_padj), 
                        color = dplyr::case_when(gene_padj < 0.05 ~ "grey",
                                                 gene_padj < 0.1 ~ "#FC4E2A",
                                                 gene_padj >= 0.1 ~ "#EEB479"),
                        label = "symbol")) + 
        geom_point(size=2) +
        geom_vline(xintercept = c(-as.numeric(fold_threshold), as.numeric(fold_threshold)), 
        ...
        labs(x=expression(Delta*"PSI"), y="-log10(P-adj)") + 
        ...
        ggrepel::geom_text_repel(
            aes(label = ifelse(gene_padj < as.numeric(threshold) & deltaPSI <= -as.numeric(fold_threshold) | 
                               gene_padj < as.numeric(threshold) & deltaPSI >= as.numeric(fold_threshold),
                               symbol, "")),
                size = 3,
                color = "black",
        ...
        xlim(min(x$deltaPSI) - 0.2, max(x$deltaPSI) + 0.2) + 
